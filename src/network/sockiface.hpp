...
cl_status Sockiface::Send(int sock, std::string msg, CmdType cmd_type) {
    ...
    if (SockWrite(this->id, &cmd, sizeof(cmd)) == cl_status::ERROR) {
    ...
    if (msg.size() > 0) {
        ...
        if (SockWrite(this->id, buf, msg.size()) == cl_status::ERROR) {
            free(buf);
            ERROR_PRINT("send failed\n");
...
cl_status Sockiface::Recv(int sock) { return cl_status::SUCCESS; }
